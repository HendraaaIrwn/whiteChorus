...
export const sharpOutfitRenderer: OutfitRenderer = {
  async render(configuration) {
    ...
    const finalWebp = await sharp(publicFile(background.assetSrcs[0]!))
    ...
    const [downloadPng, thumbnailWebp, socialCharacter, sharePng] =
      await Promise.all([
    ...
    const socialJpeg = await sharp({
      ...
      .jpeg({ quality: 90 })
      .toBuffer();
    return { finalWebp, downloadPng, thumbnailWebp, socialJpeg, sharePng };
  },
};
